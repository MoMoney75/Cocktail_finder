...
@app.route('/login')
def login_page():
    """shows login form with link to signup if not an exisisting user"""
    form  = Login()
...
def login():
    ...
    #return render_template('search_ingredients.html', form=form)
...
@app.route('/handle_search_ingredients', methods=["POST"])
def get_cocktails():
    form = SearchIngredient()
    user_id = session['user_id']
    user = User.query.get_or_404(user_id)
            ...
            if data == {'drinks': 'None Found'}:
                ...
                for drink in drinks_ids:
                    ...
                    for drinks in cocktail_details['drinks']:
                        ...
                        drink_data.append({"name": name, "details": drinks}) #new
